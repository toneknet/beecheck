<?php
// Check if apiarie is selected else exit;
...
if (isset($_POST['action']) && $_POST['action'] === 'multi_comment') {
    ...
    header('Location: /hives/'. $apiary_id);
    exit;
}
...
if (isset($_POST['action']) && $_POST['action'] === 'delete_hive') {
    ...
    $stmt = $mysqli->prepare("UPDATE bi_hives h JOIN bi_apiaries a ON h.apiary_id = a.id SET h.deleted=1 WHERE h.id = ? AND a.user_id = ?");
...
?>
    <main class="app-main">
            ...
            <!-- Ny bigård -->
            <details class="card">
                <summary class="card-title">Ny kupa
                </summary>
                <!-- Ny kupa -->
            ...
            <div class="card apiary-card">
                <div class="card-header">
                    ...
                    <div class="apiary-actions">
                        <a href="<?= url('all_logs/?apiary_id=' . $apiary['id'] . '&hive_id=0&sort=date_desc') ?>" class="button">Loggar för bigård</a>
                        <!-- <form method="post">
                            ...
                            <?php foreach ($apiary['hives'] as $hive): ?>
                                <div class="hive-item">
                                    ...
                                    <div style="display:flex; flex-wrap:wrap; gap:0.35rem;">
                                        <a class="button" href="<?=url('hive_logs/'.$hive['id']);?>"> <!-- hive_logs/<?= $hive['id'] ?> -->
                                            Loggar
                                        </a>
                                        <form method="post">
    ...
    <?php if ($apiary['hives']): ?>
        ...
        <details >
            <summary>Gemensam kommentar för valda kupor i denna bigård</summary>
        <form method="post" style="margin-top:0.5rem; border-top:1px solid #ddd; padding-top:0.5rem;">
            <input type="hidden" name="action" value="multi_comment">
            <p>
                <label style="display:inline-block; margin-right:0.5rem; font-weight:600;">
...
<script>
function toggleAllHives(masterCheckbox) {
    // Hitta närmaste formulär (gäller den bigård du är inne i)
    const form = masterCheckbox.closest('form');
    if (!form) return;

    // Hitta alla kupa-checkboxar i just detta formulär
    const hiveCheckboxes = form.querySelectorAll('input.hive-checkbox');
